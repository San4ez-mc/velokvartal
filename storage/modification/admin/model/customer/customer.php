...
class ModelCustomerCustomer extends Model {
	public function addCustomer($data) {
      ...
      if (!empty($data['customer_id']) && defined('GKD_UNIV_IMPORT')) {
        ...
        $univimp_item_id = '';
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "customer SET " . $univimp_item_id . " customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : json_encode(array())) . "', newsletter = '" . (int)$data['newsletter'] . "', salt = '" . ((defined('GKD_UNIV_IMPORT') && !empty($data['salt'])) ? $this->db->escape($salt = $data['salt']) : $this->db->escape($salt = token(9))) . "', password = '" . ((defined('GKD_UNIV_IMPORT') && !empty($data['salt'])) ? $this->db->escape($data['password']) : $this->db->escape(sha1($salt . sha1($salt . sha1($data['password']))))) . "', status = '" . (int)$data['status'] . "', safe = '" . (int)$data['safe'] . "', date_added = NOW()");
	...
	public function editCustomer($customer_id, $data) {
		...
		if ($data['password']) {
			$this->db->query("UPDATE " . DB_PREFIX . "customer SET salt = '" . ((defined('GKD_UNIV_IMPORT') && !empty($data['salt'])) ? $this->db->escape($salt = $data['salt']) : $this->db->escape($salt = token(9))) . "', password = '" . ((defined('GKD_UNIV_IMPORT') && !empty($data['salt'])) ? $this->db->escape($data['password']) : $this->db->escape(sha1($salt . sha1($salt . sha1($data['password']))))) . "' WHERE customer_id = '" . (int)$customer_id . "'");
		}
