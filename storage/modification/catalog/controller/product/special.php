...
class ControllerProductSpecial extends Controller {
	public function index() {
		$this->load->language('product/special');
			...
			$data['menu_open_special'] = (!empty($megamenu_setting['config_menu_open_special']) ? $megamenu_setting['config_menu_open_special'] : 0);
			$data['main_menu'] = $megamenu_setting['main_menu_selection'];
			$this->load->language('cyberstore/lang');
			$data['lang_id'] = $this->config->get('config_language_id');
			$data['ns_on_off_special_page_model_product'] = (!empty($this->config->get('config_on_off_special_page_model_product')) ? 1 : 0);
			$data['ns_on_off_special_page_description'] = (!empty($this->config->get('config_on_off_special_page_description')) ? 1 : 0);
			$data['status_additional_image_hover_sp'] = (!empty($this->config->get('status_additional_image_hover_sp')) ? 1 : 0);
			$data['ns_on_off_special_page_rating'] = (!empty($this->config->get('config_on_off_special_page_rating')) ? 1 : 0);
			$data['ns_on_off_special_page_quantity_reviews'] = (!empty($this->config->get('config_on_off_special_page_quantity_reviews')) ? 1 : 0);
			$data['ns_on_off_special_page_fastorder'] = (!empty($this->config->get('config_on_off_special_page_fastorder')) ? 1 : 0);
			$data['ns_on_off_special_page_wishlist'] = (!empty($this->config->get('config_on_off_special_page_wishlist')) ? 1 : 0);
			$data['ns_on_off_special_page_compare'] = (!empty($this->config->get('config_on_off_special_page_compare')) ? 1 : 0);
			...
			$data['config_price_list_view_on_off_special_page'] = (!empty($this->config->get('config_price_list_view_on_off_special_page')) ? 1 : 0);
		...
		foreach ($results as $result) {
			...
			if($data['status_additional_image_hover_sp'] =='1'){
