...
class ModelCatalogCyberProductTabs extends Model {
    public function getTabs($product_id) {
        ...
        $tabs_ns_id =  implode(',', array_map(function ($value) {return $value['tabs_ns_id'];}, $product_tabs));
    ...
    private function getTabsViewAll($tabs_ns_id,$product_id) {
        ...
        if(!empty($tabs_ns_id)){
        $sql .= " AND tn.tabs_ns_id NOT IN ($tabs_ns_id)";
        }
        ...
        if ($query->rows) {
            foreach ($query->rows as $result) {
                ...
                if($show_categories){
                    $query_c = $this->db->query("SELECT * FROM `". DB_PREFIX ."product_to_category` WHERE category_id IN( ". implode(',', $show_categories) ." ) AND product_id = '". (int)$product_id ."'");
                    if($query_c->row){
                      $status_tab = true;
                    }
                }
                ...
                if($show_attributes){
                    $query_a = $this->db->query("SELECT * FROM `". DB_PREFIX ."product_attribute` WHERE attribute_id IN( ". implode(',', $show_attributes) ." ) AND product_id = '". (int)$product_id ."' AND language_id = '" . (int)$this->config->get('config_language_id') . "'");
                    if($query_a->row){
                      $status_tab = true;
                    }
                }
