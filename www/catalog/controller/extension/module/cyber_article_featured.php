<?php
class ControllerExtensionModuleCyberArticleFeatured extends Controller {
	public function index($setting) {
		...
		if (!empty($setting['article'])) {
			...
			foreach ($articles as $article_id) {
				$article_info = $this->model_cyberstore_article->getArticle($article_id);
		...
		$data['module'] = $module++;
		if ($data['articles']) {
			return $this->load->view('extension/module/cyber_article_featured', $data);
		}
		}
	}
}